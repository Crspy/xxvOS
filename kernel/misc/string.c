...
#define ONES ((size_t)-1/UCHAR_MAX) 			// 0x0101010101010101
#define HIGHS (ONES * (UCHAR_MAX/2+1)) 			// 0x8080808080808080
...
size_t strlen(const char * s) {
	// faster than the naive strlen implementation

	const char * a = s;
	const size_t * w;
	// keep incrementing with length = remainder of  s mod ALIGN as long as *s != null term
	...
	// keep incrementing 8 bytes in each iteration while checking for null terminator in one of the bytes of *w
	for (w = (const void *)s; !HASZERO(*w); w++);

	// increment s starting from w until we hit the null terminator byte
